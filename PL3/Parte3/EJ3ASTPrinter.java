 ...
 */
public class EJ3ASTPrinter extends EJ3ParserBaseListener {
    ...
    @Override public void exitPrograma(EJ3Parser.ProgramaContext ctx) { indent--; }
    ...
    // -----------------------
    private String toBool(EJ3Parser.CondicionContext c) { return toBoolO(c.bool_o()); }

    private String toBoolO(EJ3Parser.Bool_oContext c) {
        List<EJ3Parser.Bool_yContext> ys = c.bool_y();
    ...
    private String toBoolY(EJ3Parser.Bool_yContext c) {
        List<EJ3Parser.Bool_noContext> ns = c.bool_no();
    ...
    private String toBoolNo(EJ3Parser.Bool_noContext c) {
    ...
    private String wrapBoolNo(EJ3Parser.Bool_noContext child) {
    ...
    private String toCmp(EJ3Parser.ComparacionContext c) {
        String left  = toOperandoCmp(c.operando_cmp(0));
        String right = toOperandoCmp(c.operando_cmp(1));
        return left + " " + c.operador_relacional().getText() + " " + right;
    }

    private String toOperandoCmp(EJ3Parser.Operando_cmpContext ctx) {
        // Si es una expresión aritmética, usamos la impresión normal de expresiones
        if (ctx.expresion() != null) {
            return toExpr(ctx.expresion());
        }
        // Si es un booleano (verdadero, falso o ID), lo imprimimos tal cual
    ...
    // -----------------------
    private String toExpr(EJ3Parser.ExpresionContext c) {
        // Recorremos los hijos para preservar exactamente el orden de + / -
        StringBuilder b = new StringBuilder();
        for (int i = 0; i < c.getChildCount(); i++) {
            ParseTree ch = c.getChild(i);
            if (ch instanceof EJ3Parser.TerminoContext) {
                b.append(toTerm((EJ3Parser.TerminoContext) ch));
            } else {
    ...
    private String wrapFactor(EJ3Parser.FactorContext f) {
        if (f.PAREN_ABRE() != null || f.atomo() != null) return toFactor(f);
        return "(" + toFactor(f) + ")";
    }

    private String toAtom(EJ3Parser.AtomoContext c) {
    ...
    // -----------------------
    private String toGeneral(EJ3Parser.Expr_generalContext c) {
        if (c.expresion() != null) {
